...
def _analyse_index_fund_flow(index_code):
    ...
    for stock_code in stock_code_list:
        fund_flow_list = _get_fund_flow(stock_dict[stock_code])
        fund_flow = fund_flow_list[len(fund_flow_list) - 1]
        fund_flow_seg = fund_flow.split(",")
        flow_zl = float(fund_flow_seg[1])
        flow_xd = float(fund_flow_seg[2])
        flow_zd = float(fund_flow_seg[3])
        flow_dd = float(fund_flow_seg[4])
        flow_cdd = float(fund_flow_seg[5])
...
def _analyse_all_index_fund_flow():
    index_list = _get_pre_install_index_list();
    for index in index_list:
        flow = _analyse_index_fund_flow(index[0])
